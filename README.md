...
Because the guard is wired as a `pre` script, it cannot be skipped by forgetting a flag. A bare `npx prisma migrate dev` never runs npm's `pre` scripts, so `backend/prisma.config.ts` applies the same check inside the Prisma CLI itself. Every command that can write to a database — `migrate dev`, `migrate reset`, `migrate resolve`, `db push`, `db execute`, `db seed` and `studio` — is refused unless every database URL (including a `db execute --url`) is localhost, however Prisma is started. `generate` and read-only commands are not affected. Still use the npm scripts: they're what load `backend/.env.dev`.

**Why `prisma migrate deploy` is deliberately not guarded.** The guard's threat model is an *accidental destructive command from a dev machine* — a reset, push or Studio edit aimed at production by a stray URL. `migrate deploy` is none of those: it is forward-only and non-destructive (it applies committed migrations that haven't run yet and never drops or resets anything), and it is the sanctioned production path that deployments and CI use. Guarding it would block the legitimate route without protecting against anything the guard exists for.
...
**Production credentials never live on a laptop.** `backend/.env` used to hold the production database connection, and everything that loaded it reached production by default — `prisma migrate dev`, the demo seed, and `npm run dev`, which also starts the Plaid sync scheduler. Each got its own guard; the file was the cause. Its `DATABASE_URL` and `DIRECT_URL` are now placeholders on a `.invalid` host, which can never resolve, so anything that falls back to that file fails loudly instead of connecting somewhere real. The same goes for `PLAID_SECRET`, `CLERK_SECRET_KEY` and `ENCRYPTION_KEY`. `ENCRYPTION_KEY` decrypts live Plaid access tokens, and local work never needs any of the three: it runs on demo data and makes no Plaid calls. Their placeholders are deliberately invalid (the encryption key isn't hex), so anything that loads them fails at startup. Production credentials exist only in Railway's environment, and the deployed app reads no `.env` file. `npm run dev` itself runs the local-db guard first (`predev`) and loads `backend/.env.dev`.
